...
func JobGet(w http.ResponseWriter, r *http.Request) {
	...
	// get job details
	job, err := DB.SelectSQL(CONSTANT.JobsTable, []string{"id", "company_id", "team_id", "name", "description", "employment_type", "salary", "location_id", "remote_option", "status", "created_at"}, map[string]string{"id": r.FormValue("job_id")})
	if err != nil {
